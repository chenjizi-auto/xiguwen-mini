...
Page({
  ...
  onLoad(options) {
    ...
    wx.setNavigationBarTitle({ title: '查看明细' })
    this.fetchDetail()
  },
  ...
  async fetchDetail() {
    ...
    try {
      ...
      if (!res || res.code !== 0) throw new Error((res && (res.message || res.msg)) || '加载明细失败')
      this.setData({
        loading: false,
        totalPrice: formatPrice(res && res.zongji),
        groups: normalizeGroups(res.data)
      })
    } catch (err) {
      this.setData({ loading: false, errorText: err && err.message ? err.message : '加载明细失败' })
    }
  },
