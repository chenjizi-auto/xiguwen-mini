...
function normalizeGallery(info = {}) {
  ...
  const urls = list
    .map(item => absoluteUrl(item && (item.photourl || item.url)))
    .filter(Boolean)
...
function normalizeCaseInfo(info = {}, typeMap = {}, environmentMap = {}, attentionCount = 0, commentCount = 0) {
  ...
  const title = safeText(info.title, '未命名案例')
  return {
...
Page({
  ...
  async fetchDetail() {
    ...
    try {
      ...
      const detailData =
        detailRes.status === 'fulfilled' && detailRes.value && detailRes.value.code === 0
      ...
      const typeMap =
        typeRes.status === 'fulfilled' && typeRes.value && typeRes.value.code === 0
          ? buildTypeMap(typeRes.value.data)
          : {}
      const environmentMap =
        environmentRes.status === 'fulfilled' && environmentRes.value && environmentRes.value.code === 0
          ? buildTypeMap(environmentRes.value.data)
          : {}

      const comments = normalizeComments(detailData.pinglun)
      const detail = normalizeCaseInfo(
        detailData.info || {},
        typeMap,
        environmentMap,
        detailData.pinglunshu,
        comments.length
      )
      const merchant = normalizeMerchant(detailData.user || {}, asNumber(detailData.userf, 0) === 1)
      const teams = normalizeTeam(detailData.team)
      const relatedCases = normalizeRelated(detailData.gdanli)
      ...
      this.setData({
        loading: false,
        errorText: err && err.message ? err.message : '加载案例详情失败'
      })
    }
  },
  ...
  onPriceDetailTap() {
    if (!this.caseId) return
    wx.navigateTo({
      url: `/pages/case/priceDetail/index?id=${this.caseId}`
    })
  },

  onMerchantAvatarError() {
    this.setData({
      'merchant.head': DEFAULT_AVATAR
    })
  },

  onTeamAvatarError(e) {
    const index = asNumber(e && e.currentTarget && e.currentTarget.dataset.index, -1)
    if (index < 0) return
    this.setData({
      [`teams[${index}].head`]: DEFAULT_AVATAR
    })
  },
  ...
  onCommentAvatarError(e) {
    ...
    if (index < 0) return
    this.setData({
      [`comments[${index}].head`]: DEFAULT_AVATAR
    })
  },
  ...
  async onFollowTap() {
    ...
    try {
      ...
      this.setData({
        'merchant.followed': !merchant.followed
      })
      wx.showToast({
        title: merchant.followed ? '已取消关注' : '已关注',
        icon: 'success'
      })
    } catch (err) {
      wx.showToast({
  ...
  onShareAppMessage() {
    ...
    return {
      title: detail.title || '案例详情',
      path: `/pages/case/detail/index?id=${encodeURIComponent(safeText(detail.id || this.caseId))}`,
