...
Page({
  data: {
    ...
    back: emptyImageSlot(),
    handheld: emptyImageSlot()
  },

  onLoad() {
    const hasMallToken = !!wx.getStorageSync('token')
    if (!xgwAuth.isLogined() && !hasMallToken) {
  ...
  async fetchStatus() {
    ...
    try {
      const res = await network.xgwGetPersonCertification({})
      wx.hideLoading()
      if (!res || res.code !== 0) {
        wx.showToast({
      ...
      if (state === STATUS_ON) {
        await this.showNotice('资料提交成功！', '我们会尽快完成您提供的个人认证资料审核～', '我知道了')
        wx.navigateBack()
        return
      }
  ...
  validateForm() {
    ...
    if (!getImageSource(this.data.handheld)) {
      wx.showToast({ title: '请上传手持身份证照', icon: 'none' })
      return false
    }
    return true
  },
    ...
    try {
      if (this.data.legacyMode) {
        const res = await WXAPI.idcardCheck(
          ...
          String(this.data.idcard || '').trim().toUpperCase()
        )
        wx.hideLoading()
        if (!res || res.code !== 0) {
          wx.showToast({
      ...
      const shou_chi_SFZ = await this.uploadSlot('handheld', '手持身份证照')
      const res = await network.xgwSubmitPersonCertification({
