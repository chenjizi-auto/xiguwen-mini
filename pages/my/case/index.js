...
function getStatusMeta(item = {}) {
  ...
  if (status === 1) return { text: '审核中', tone: 'pending' }
  if (status === 2) {
    return {
  ...
  if (status === 3) return { text: '审核未通过', tone: 'danger' }
...
function normalizeItem(item = {}) {
  ...
  return {
    ...
    title: safeText(item.title, '未命名案例'),
    description: safeText(item.weddingdescribe, ''),
    cover: safeText(item.weddingcover, DEFAULT_COVER) || DEFAULT_COVER,
    weight: safeText(item.weigh, ''),
    weddingTime: safeText(item.weddingtime, ''),
...
Page({
  ...
  goAdd() {
    wx.navigateTo({ url: '/pages/my/case/form/index' })
  },

  goDetail(e) {
    const id = asNumber(e.currentTarget.dataset.id, 0)
    if (!id) return
    wx.navigateTo({ url: `/pages/my/case/detail/index?id=${id}` })
  },
  ...
  async loadList(isRefresh) {
    ...
    try {
      ...
      const res = await network.xgwCaseList({
        p: page,
        rows: this.data.rows,
