...
function normalizeItem(item = {}) {
  ...
  return {
    ...
    title: safeText(item.title, '未命名视频'),
    description: safeText(item.titlea || item.title, ''),
...
Page({
  ...
  goDetail(e) {
    ...
    if (!id) return
    wx.navigateTo({
      url: `/pages/my/video/detail/index?id=${id}`
    })
  },
  ...
  async loadList(isRefresh) {
    ...
    try {
      ...
      const res = await network.xgwVideoList({
        p: page,
        rows: this.data.rows,
